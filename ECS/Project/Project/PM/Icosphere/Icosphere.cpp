...
void Icosphere::generateIcosphereGeometryPerVertex(std::vector<std::pair<std::string, std::shared_ptr<GeometryData>>> & newGeometryDataList, std::string name)
{
	...
	for (auto &current_vertex : vertices)
	{
		if (roughness != 0)
		{
			current_vertex.x = (float)(current_vertex.x + RNGs::doubleRand(0.0f, 1.0f) / 2.0f);
			current_vertex.y = (float)(current_vertex.y + RNGs::doubleRand(0.0f, 1.0f) / 2.0f);
			current_vertex.z = (float)(current_vertex.z + RNGs::doubleRand(0.0f, 1.0f) / 2.0f);
		}
	}
